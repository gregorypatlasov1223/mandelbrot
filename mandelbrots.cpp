    ...
    for (int y = 0; y < kHeight; y++)
    {
        ...
        for (int x = 0; x < kWidth; x++)
        {
            ...
            while (n_of_iter < kMaxIter)
            {
                ...
                float p_re_new = p_re * p_re - p_im * p_im + p0_re;  // Re(z^2 + c)
                float p_im_new = 2.0f * p_re * p_im + p0_im;         // Im(z^2 + c)
                p_re = p_re_new;
                p_im = p_im_new;
                n_of_iter++;
            }
...
void QuadMandelbrot(RGBQUAD* videoBuf)
{
    ...
    for (int y = 0; y < kHeight; y++)
    {
        ...
        for (int x = 0; x < kWidth; x += kVecWidth)
        {
            ...
            int n_of_iters[kVecWidth] = {};      // массив счетчиков итераций, пока фиг знает как назвать
            ...
            for (int n = 0; n < kMaxIter; n++)
            {
                ...
                for (int i = 0; i < kVecWidth; i++)  // if (cmp[i]) x_arr[i] = x2[i] - y2[i] + x0_arr[i] <- меньше работы
...
void VectorMandelbrot(RGBQUAD* videoBuf)
{
    ...
    for (int y = 0; y < kHeight; y++)
    {
        ...
        for (int x = 0; x < kWidth; x += 8)
        {
            ...
            x0_vec = _mm256_add_ps(x0_vec, eight_dx_arr); // следующая пачка пикселей (x+8 ... x+15)
        }
    }
    return;
}
